...
class poker_parser_holdemmanager extends poker_parser {
    ...
    public function info(){
         $this->wy_info['parser_name'] = "HoldemManager";
         ...
         preg_match("#$this->waluta$this->kasa/$this->waluta$this->kasa Blinds (Fix|No|Pot) Limit (Holdem|Omaha)#", $this->dane, $r );
         if ($r[2]>0){$this->wy_info['sb']['stawka'] = preg_replace('{([^0-9.])}','',$r[2]);}
         if ($r[4]>0){$this->wy_info['bb']['stawka'] = preg_replace('{([^0-9.])}','',$r[4]);}
...
//------------------------- RUCHY ----------------------------------------------
         ...
         foreach ($tab as $value){
             ...
             $id = $this->getIdUserByLogin($r[1]);
             
             if ($id>0){
                 $this->wy_info["seat$id"]['k1']=$this->jakakarta($r[2]);
                 $this->wy_info["seat$id"]['k2']=$this->jakakarta($r[3]); 
             }
         ...
         preg_match('#\*\* Dealing flop \*\* \[ '.$this->karta.', '.$this->karta.', '.$this->karta.' \]'.$this->ciagruchow.'\n\*#', $this->dane, $r );  
         $this->ruchyGRC( split("\n", $r[4]),'flop');
         
         preg_match('#\*\* Dealing turn \*\* \[ '.$this->karta.' \]'.$this->ciagruchow.'\n\*#', $this->dane, $r );  
         $this->ruchyGRC(  split("\n", $r[2]),'turn');
         
         preg_match("#\*\* Dealing river \*\* \[ $this->karta \]$this->ciagruchow\n\*#", $this->dane, $r );  
         $this->ruchyGRC( split("\n", $r[2]),'river');
...
//------------------------- SUMMARY --------------------------------------------
         ...
         $id = 0; foreach ($tab as $value){
             ...
             preg_match("#$this->gracz (shows|mucks) \[ $this->karta, $this->karta \]#", $value, $r );            
             ...
             preg_match("#$this->gracz collected \[ $this->waluta$this->kasa \]#",$value,$r);             
             if ($r[1]!=''){
